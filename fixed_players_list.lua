...
--// - Draggable title bar ONLY (fixed)
...
win.Active = true
win.Draggable = false  -- ห้ามลากทั้งหน้าต่าง
win.Parent = sg
corner(win, 18)
...
local function makeRow(plr)
    ...
    copyUser.Position = UDim2.new(1, -200, 0, 36)
    ...
    copyId.Position = UDim2.new(1, -100, 0, 36)
    copyId.Parent = row
...
-- บันทึกตำแหน่งปุ่มลอยเมื่อลาก
floatBtn:GetPropertyChangedSignal("Position"):Connect(function()
    getgenv().PrettyList_Toggle_Pos = floatBtn.Position
end)
